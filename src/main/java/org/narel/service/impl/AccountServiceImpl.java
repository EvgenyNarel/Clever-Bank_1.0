...
public class AccountServiceImpl implements AccountService {
    ...
    @Transactional
    @Override
    public OperationDto accrueInterest(Account account, int percent) {
        BigDecimal amount = account.getAmount();
        if (amount.compareTo(BigDecimal.valueOf(0)) > 0) {
            BigDecimal accrualPercents = amount.multiply(BigDecimal.valueOf(percent / 100d));

            account.setAmount(amount.add(accrualPercents));

            Operation operation = Operation.builder()
                    .id(UUID.randomUUID())
                    .recipientId(account.getId())
                    .kind(OperationKind.ACCRUAL)
                    .amount(accrualPercents)
                    .operationDate(Instant.now())
                    .build();
